...
export async function chessAiEngineApi_initChessAiEngine(): Promise<void> {
  ...
  if (internalSettings.AIEngineUsesSocket) {
    ...
    chessAiEngineUrl = 'wss:' + process.env.VITE_APP_CHESS_ENGINE_API_URL;
    // Set up socket communication:
    chessAiEngine_socket = new WebSocket(chessAiEngineUrl);
    chessAiEngine_socket.onmessage = (event): void => {
    ...
    chessAiEngineUrl = 'https:' + process.env.VITE_APP_CHESS_ENGINE_API_URL;
    ...
    try {
      ...
      chessAiEngineUrl =
        process.env.VITE_APP_FETCH_CORS_PROXY_SERVER + chessAiEngineUrl;
      try {
        const move = await getAISmartMove_fetch(true);
        console.log(
