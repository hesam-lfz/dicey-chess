...
const localStorageKeyPrefix = process.env.VITE_APP_NAME;
const appVersion = process.env.VITE_APP_VERSION;

// Cached saved games data retrieved from database:
let cachedSavedGames: SavedGame[] | undefined;

// Retrieve saved settings from local storage:
export async function storageApi_loadSettings(): Promise<Settings | null> {
  // -- FIXME: REMOVE SOON --
  ...
  if (
    ...
    await AsyncStorage.removeItem(localStorageKeyPrefix + '-settings');
    await AsyncStorage.removeItem(localStorageKeyPrefix + '-games');
  }
...
export async function storageApi_saveSettings(
  settings: Settings
): Promise<void> {
